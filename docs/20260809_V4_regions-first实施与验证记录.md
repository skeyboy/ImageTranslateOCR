...
其中 V4 的 `renderSlots` 是服务端权威绘制区域，可以把多行 OCR 槽合并成一个段落矩形，
但所有槽位的联合外包围必须与成员 OCR 的 `anchorBounds` 一致。`sourceCoverSlots` 仍必须逐项
等于 OCR 的 componentBounds/bounds。这样既允许整段回流，又不允许服务端扩大到原文区域之外。
...
### 6.2 Rust 官方文档长段落回贴修复

验证页面：

- `https://doc.rust-lang.org/book/ch00-00-introduction.html#introduction`
- 页面主题：深色技术文档
- 真机当前视口：`Who Rust Is For`、`Teams of Developers`、长正文和项目列表

第一次回归中，服务端审计记录 `0caa67b6-3314-4762-b3f7-8d51d07a6c07` 已成功返回 6 个
V4 权威结果，但 Android 只回贴 4 个短组，长正文仍保留英文。联合请求、响应和端侧日志后确认
有两个独立问题：

1. V4 权威响应返回后，端侧再次执行 SmartAssist，重建了 groupId、renderSlots 和 layoutHint，
   导致服务端 DSL 被覆盖。
2. Android V4 解析器仍沿用 V3 几何约束，要求 `renderSlots` 与逐行 OCR 框完全相等。服务端为
   12/13 行正文返回合法的单矩形段落槽时，响应被判为
   `SELF_HOSTED_REQUEST_FAILED` 并降级到端侧翻译；长正文保护策略随后保留了英文原文。

对应修复：

1. `SELF_HOSTED_V4` 跳过翻译后的第二次 SmartAssist，端侧上传前建议组仍保留。
2. V4 接受服务端权威合并 `renderSlots`，校验其联合边界与 OCR 成员外包围一致。
3. `sourceCoverSlots` 继续严格匹配逐行 OCR 几何，防止遮罩侵占无关页面区域。
4. 单矩形长文本优先完整回流，依次尝试有限字号/间距压缩、低字号紧急布局和多槽联合回退；
   此阶段不因 `preferredMaxLines` 偏小而直接恢复原文。
5. 自建请求异常记录实际异常信息，避免协议校验失败被误判成网络超时。

最终真机记录：`4b38aa14-8438-4018-874f-dcd326fe0fc8`。

| 指标 | 结果 |
| --- | --- |
| schema / provider | V4 / `self-hosted-qwen-regions-first-v4` |
| OCR 原子区域 | 29（服务端审计为 27 个正文区域，端侧 recognized 包含额外可见文字） |
| 客户端建议组 | 6 |
| 服务端计划组 | 6 |
| 服务端权威可执行组 | 6 |
| Android patches | 6 |
| Android failed | 0 |
| 端到端耗时 | 57.4 秒 |
| 最大正文组 | 13 个成员、188 个译文字、单个 RECT renderSlot |

结论：V4 已达到本阶段“所有参与翻译的大段落优先完成回贴”的目标。13 行正文、普通段落和
列表均成功回贴，不再出现大面积英文原文保留。当前遮罩采用较宽松的整段矩形，视觉精度仍有
提升空间，但不应通过重新引入拒绝回贴来解决。
...
结合 Rust 官方文档的最新真机结果，后续优化按以下顺序执行：

1. **保持完整回贴基线**：为文章、聊天、技术文档建立 `failed=0` 回归样例；后续任何视觉
   优化不得让长正文重新恢复原文。
2. **遮罩外形收紧**：保留单个正文 renderSlot 用于文字回流，但根据 `sourceCoverSlots` 生成
   行级或连续多边形擦除区域，避免矩形空白处被整块灰色覆盖。
3. **字号与行距拟合**：优先从 OCR 行高、原文字号中估算基准字号，再在服务端
   `minimumTextScale/lineSpacingMultiplier` 范围内搜索；减少当前译文字号偏大、行距偏松问题。
4. **译文占用预测**：服务端使用字符数、语言膨胀率、槽宽和源行高生成候选字号，而 Android
   以真实字体测量做最终裁决；把实际使用字号、行数和剩余空间写入调试回传。
5. **列表和图文混排**：列表符号、图片和正文分别建立不可侵占区域；正文可在多个 flow slot
   中连续排版，但遮罩不得覆盖项目符号、图片或相邻栏。
6. **视觉一致性对比**：Admin 同屏比较 OCR 原图、服务端预测布局和端侧实际回贴截图，量化
   cover precision、原文残留率、额外遮挡率和译文完整率。
7. **最后恢复约束**：只有完成上述量化后，才逐步提高最小字号或收紧槽位；空间仍不足的大块
