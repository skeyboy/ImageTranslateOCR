# 图片文字翻译与替换调校记录

本文档按时间顺序记录 OCR、翻译、文字擦除和译文回填的修改原因、效果与验证信息，供后续效果对比和问题回溯使用。

## 项目目标

将图片中的中文识别并翻译为英文，在尽量保留原图布局、背景、字号层级和文字颜色的前提下擦除原文并写入译文。

## 验证环境

| 项目 | 当前配置 |
| --- | --- |
| 操作系统 | macOS 26.5.2 arm64 |
| Android Studio JBR | OpenJDK 21.0.10 |
| Android Gradle Plugin | 8.7.3 |
| Gradle Wrapper | 8.9 |
| Kotlin | 2.0.21 |
| compileSdk / targetSdk | 34 / 34 |
| OCR | ML Kit Chinese Text Recognition 16.0.1 |
| 翻译 | ML Kit Translation 17.0.3 |
| 图像修复 | OpenCV 4.5.3 Telea Inpaint |
| 测试设备画面 | Pixel 3 模拟器截图 |

命令行构建：

```bash
JAVA_HOME="/Applications/Android Studio.app/Contents/jbr/Contents/Home" ./gradlew :app:assembleDebug
```

## 2026-07-20：处理链路稳定化

提交：`1d41503 fix(android): stabilize image translation pipeline`

### 原始问题

- OpenCV 和图片处理运行在主线程，大图处理可能阻塞界面。
- 使用原文字符串作为译文 Map 的键，相同文字可能互相覆盖。
- 译文被限制为单行，较长英文会缩小到难以阅读。
- 精确遮罩存在 Android `Rect` 与 OpenCV `Rect` 类型混用问题。
- 小尺寸 OCR 区域可能无法满足自适应阈值的 block size 要求。
- OpenCV 异步初始化失败时仍可能被标记为成功。
- ML Kit 回调可能在协程取消后重复恢复 continuation。

### 修改内容

- 将图片解码、缩放、OpenCV 修复和 Canvas 绘制移到后台线程。
- 将译文与每个 OCR 区域直接绑定。
- 初步增加 `StaticLayout` 多行排版和字号适配。
- 修正精确遮罩的坐标裁剪、动态阈值与 Mat 释放。
- 修正 OpenCV `BaseLoaderCallback` 初始化回调。
- OCR 最长边处理上限由 1024 px 提高到 2048 px。
- 升级构建工具并补齐完整 Gradle Wrapper，以兼容当前 JDK 21 环境。

### 构建异常及处理

旧的 AGP 8.2.0 + Gradle 8.5 在 JDK 21 下生成 Android JDK image 时失败：

```text
ModuleTarget is malformed: platformString missing delimiter: android
```

升级至 AGP 8.7.3 + Gradle 8.9 后，`:app:compileDebugJavaWithJavac` 和 `:app:assembleDebug` 均通过。

### OpenCV 运行时异常及处理

`Utils.bitmapToMat()` 输出四通道 RGBA Mat，而 OpenCV 4.5.3 的 `Photo.inpaint()` 不接受四通道输入：

```text
Unsupported format or combination of formats
8-bit 3-channel input/output images are supported in function 'icvInpaint'
```

矩形遮罩和精确遮罩路径均改为先执行 `COLOR_RGBA2RGB`，再将三通道 Mat 传给 `Photo.inpaint()`。修复后 Debug APK 构建通过。

## 2026-07-21：第一轮视觉效果分析

### 对比素材

原图：

![中文原图](assets/2026-07-21-source.png)

稳定化版本输出结果：

![第一轮调校前的英文输出](assets/2026-07-21-before-rendering-tuning.png)

### 截图确认的问题

1. 英文长度通常大于中文，但译文被限制在中文原始行框内，正文被缩小到约原字号的一半。
2. 所有译文统一使用粗黑字体，原图中的标题、正文和紫色强调文字层级丢失。
3. 原图存在充足横向空白，但排版没有利用这些空间。
4. 短 UI 文案缺少上下文，出现 `分享分享`、`Editor`、`Day removal` 等不自然结果。
5. 文字擦除后的背景整体较干净，本轮主要问题集中在译文排版和术语质量。

## 2026-07-21：可用空间排版调校

提交：`d58282b feat(rendering): preserve source text appearance`

### 修改内容

- 不再把译文严格裁剪在中文原始边界内。
- 从 OCR 区域左侧开始使用右侧空白，并检测、避让同行的其他文字区域。
- 向下最多扩展三个原文字高度，并避让下方具有水平重叠的文字区域。
- 字号以原文字高度为基准，优先保持约 `0.9 * OCR 区域高度`。
- 最小字号限制为约 `0.62 * OCR 区域高度`，避免无限缩小。
- 使用常规字重替代统一粗体。
- 从原图区域的背景主色与前景像素差异估算文字颜色。
- 增加常见 Android UI 术语映射，如 `Share`、`Edit`、`Delete`、`Auto-rotate`。
- 其他文本仍由 ML Kit 离线翻译模型处理。

### 当前验证结果

- `git diff --check`：通过。
- `:app:compileDebugKotlin`：通过。
- `:app:compileDebugJavaWithJavac`：通过。
- `:app:assembleDebug`：通过。
- APK：`app/build/outputs/apk/debug/app-debug.apk`。
- 新排版的真机截图：待复核。当前只能确认实现和构建有效，视觉改善程度需要使用同一张原图重新处理后比较。

### 后续重点观察

- 长段落是否仍因逐行翻译产生语义割裂或行间拥挤。
- 动态扩展区域是否会覆盖图标、开关或非文字控件。
- 浅色文字、彩色文字和复杂背景下的颜色估算是否稳定。
- 底部按钮等居中文字是否需要单独判断对齐方式。
- 精确遮罩在浅色文字、深色背景和纹理背景上的擦除完整度。
- 是否需要将 OCR 行按 ML Kit TextBlock 合并后翻译，以改善长段落上下文。

## 2026-07-21：第二轮控件标签调校

提交：`8390693 fix(rendering): constrain translated control labels`

### 局部对比素材

原图局部：

![底部操作按钮原图](assets/2026-07-21-control-source.png)

可用空间排版版本的输出：

![控件标签修复前的输出](assets/2026-07-21-control-output-before-fix.png)

### 截图确认的问题

1. `分享 分享` 未命中精确术语表；模型失败或 OCR 重复时直接保留了中文。
2. `删除` 被模型误译为 `Day removal`，说明孤立短词缺少 UI 上下文。
3. `Edit` 和 `Day removal` 的前景色偏灰，与原按钮白色标签不一致。
4. 第一轮自由空间扩展适合页面正文，但不适合胶囊按钮，长译文会侵入右侧控件。
5. 底部低对比度设置项存在中文残留或错误字符，需要用常见系统短语纠正低质量 OCR 输入。

### 修改内容

- 识别“深色背景 + 较短 OCR 区域”为控件标签。
- 控件标签保持原 OCR 中心和水平边界，不再使用页面级自由扩展。
- 控件标签使用居中排版；普通页面文字继续使用左对齐动态空间。
- 前景色提取改为优先选择与背景距离最大的像素，减少抗锯齿像素导致的灰色偏差。
- 深色背景无法可靠提取前景时回退为白色，浅色背景回退为黑色。
- 对较短 OCR 文本进行关键词归一化，包含 `分享`、`编辑`、`删除` 时分别固定为 `Share`、`Edit`、`Delete`。
- 增加 `连接或断开电源时唤醒` 的系统设置译法 `Wake on power connection`。

### 当前验证结果

- `git diff --check`：通过。
- `:app:compileDebugKotlin`：通过。
- `:app:assembleDebug`：通过。
- 修复后的按钮截图：待使用同一原图重新处理后补充。

### 下一轮观察项

- `Share`、`Edit`、`Delete` 是否在按钮内保持居中且不覆盖图标。
- 白色标签是否恢复足够对比度。
- 底部电源设置是否完整替换为英文且原中文被彻底擦除。
- 深色普通背景上的正文是否会被误判为控件标签。

## 2026-07-21：乱码过滤与深色背景 OCR 增强

提交：`96bddfb feat(ocr): filter noise and enhance dark text regions`

### 问题判断

- 单次原图 OCR 对深色按钮上的浅色文字识别不稳定，错误文字会继续进入翻译模型。
- 仅靠术语映射无法覆盖所有 OCR 错字。
- 英文翻译结果可能残留汉字或包含大量符号，继续回填会形成明显乱码。
- 旧流程在翻译失败后仍擦除原图并重绘 OCR 原文，可能把原本正确的像素替换成识别错误文字。

### 修改内容

- 保留原图 OCR，并增加一次整图反色 OCR。
- 反色候选仅用于原图判断为深色背景的区域，避免普通浅色页面产生重复识别框。
- 原图和反色结果按区域重叠率合并；深色背景优先采用反色结果，其他区域按文本质量评分选择。
- OCR 候选要求至少 60% 的非空白字符为字母、数字或汉字，过滤符号型噪声。
- 对长度不超过 8 的 UI 短词执行编辑距离匹配，允许一个 OCR 字符错误后仍匹配已知术语。
- ML Kit 翻译结果必须包含足够比例的字母或数字，并且不能残留汉字。
- 未通过翻译质量检查的区域不再进入遮罩和绘制步骤，直接保留原图像素。

### 取舍

- 双 OCR 会增加识别阶段耗时和一次同尺寸临时 Bitmap 的内存占用。
- 当前策略倾向于保守：无法确定译文质量时宁可保留中文，也不写入乱码。
- 文本质量规则只能过滤字符结构异常，无法判断语义正确性；常见 UI 术语仍需要结合实际样本迭代。

### 当前验证结果

- `git diff --check`：通过。
- `:app:compileDebugKotlin`：通过。
- `:app:assembleDebug`：通过。
- 本次构建实际使用本机未提交的 Gradle Wrapper 8.13 配置。
- 深色区域识别准确率和过滤效果：待使用相同样本重新截图复核。

### 下一轮观察项

- 双 OCR 后 `Share`、`Edit`、`Delete` 的源文字是否稳定。
- 是否仍出现英文中夹杂汉字或无意义符号。
- 被过滤区域是否正确保留完整中文原图，而不是留下擦除痕迹。
- OCR 阶段增加的耗时是否处于可接受范围。

## 2026-07-21：动态翻译、三路 OCR 与自适应擦除

提交：`e6d6b27 feat(pipeline): add adaptive OCR translation and inpainting`

### 动态翻译

- 新增 ML Kit Language Identification 17.0.6。
- 每段 OCR 文本先识别 BCP-47 源语言，目标语言当前统一为英文。
- 已经是英文的文字保持不变；其他 ML Kit 支持语言按需创建“源语言 → 英文”Translator。
- Translator 按语言对缓存，模型只在首次使用时下载，并在 Activity 销毁时统一关闭。
- 短文本返回 `und` 时根据是否包含汉字回退为中文或目标语言。
- 中文 UI 术语规则只在源语言为中文、目标语言为英文时启用。

### OCR 精度增强

- 原图 OCR：保留颜色和正常对比度信息，作为主要结果。
- 灰度高对比 OCR：增强低对比度正文和浅灰系统文字。
- 反色 OCR：增强深色按钮或深色背景上的浅色文字。
- 三路候选根据矩形重叠率、背景明暗和文本质量评分合并。
- 反色结果只允许补充深色区域，防止浅色页面出现重复 OCR 框。

ML Kit 中文 OCR 模型仍使用官方 bundled `16.0.1`。官方说明输入字符应至少约 16×16 px，超过约 24×24 px 通常不会继续提高准确率，因此本项目把改进重点放在对比度和极性预处理，而不是继续无上限放大图片。

### OpenCV Inpaint 增强

- 精确遮罩同时生成深色文字遮罩和浅色文字遮罩。
- 根据前景像素占区域面积的比例选择更接近文字笔画分布的遮罩。
- 对极小区域保留 Otsu 阈值回退。
- 选定遮罩后继续执行形态学膨胀和 Telea Inpaint。
- 解决旧实现只使用 `THRESH_BINARY_INV`、对白字深底擦除不完整的问题。

### 构建依赖整理

- `compileSdk` 调整为本机已安装且 AGP 8.7.3 支持的 API 35。
- 新增 `com.google.mlkit:language-id:17.0.6`。
- AndroidX Core 固定为 1.16.0，Lifecycle 固定为 2.9.2。
- Coroutines 固定为与 Kotlin 2.0.21 元数据兼容的 1.8.1。
- 移除源码未使用的 OkHttp 和 Gson，避免无关依赖提升 compileSdk 要求。

### 取舍与限制

- 三次 OCR 会明显增加识别耗时，峰值内存包含两个依次创建并释放的同尺寸临时 Bitmap。
- ML Kit Language Identification 对非常短的词可能返回 `und`，已增加文字脚本回退，但不能完全消除误判。
- 目标语言目前仍是英文；后续可增加 UI 选择器，将目标语言作为参数传入现有动态翻译接口。
- ML Kit 离线翻译适合常见表达，但不具备云端大模型的完整上下文能力。

### 当前验证结果

- `git diff --check`：通过。
- `:app:checkDebugAarMetadata`：通过。
- `:app:compileDebugKotlin`：通过。
- `:app:compileDebugJavaWithJavac`：通过。
- `:app:assembleDebug`：通过。
- 新 OCR 和遮罩效果：待使用相同样本进行真机截图对比。

## 2026-07-21：Android 依赖严格锁定

提交：`e658304 fix(build): strictly pin Android-compatible dependencies`

### 问题

依赖声明再次被升级为 AndroidX Core 1.19.0。该版本要求 compileSdk 37 和 AGP 9.1，而当前稳定构建环境为 compileSdk 35 和 AGP 8.7.3；Lifecycle 2.11.0 与 Coroutines 1.11.0 还会引入 Kotlin 2.2 元数据，与 Kotlin 插件 2.0.21 不兼容。

### 处理

- 使用 Gradle `strictly` 将 AndroidX Core 锁定为 1.16.0。
- 将 Lifecycle 严格锁定为 2.9.2。
- 将 Coroutines 严格锁定为 1.8.1。
- 严格约束会阻止传递依赖或 IDE 升级建议将构建静默提升到不兼容版本。

### 验证

- `dependencyInsight` 确认 `core-ktx:{strictly 1.16.0} -> 1.16.0`。
- 执行 `:app:clean :app:assembleDebug`，构建通过。

## 2026-07-21：深色按钮单点误识别修复

提交：`edc4ac2 fix(ocr): prevent bad dark-pass candidate overrides`

### 对比素材

原图局部：

![深色按钮原图](assets/2026-07-21-dark-button-source.png)

异常结果：

![分享按钮被识别为 Minutes](assets/2026-07-21-dark-button-bad-result.png)

### 根因

同一组深色按钮中，`编辑` 和 `删除` 的反色 OCR 候选正确，但 `分享` 的反色候选错误。旧融合逻辑只要判断为深色区域，就无条件用反色候选覆盖原图或高对比度候选，导致原本可能正确的 `分享` 被替换为时间类文本，随后产生 `<Minutes`。这是候选融合优先级错误，不是单纯的翻译错误。

### 修改内容

- 反色 OCR 不再无条件覆盖深色区域的现有候选。
- 深色增强候选的质量分必须比现有结果至少高 0.08 才能替换。
- 普通高对比候选继续使用更严格的 0.15 提升门槛。
- 文本质量评分增加首尾异常符号惩罚。
- 中英文混合脚本按占比扣分，降低乱码候选优先级。
- 翻译结果以 `<`、`>`、`=` 或 `|` 开头时判定异常，保留原图区域。

### 验证

- `git diff --check`：通过。
- `:app:compileDebugKotlin`：通过。
- `:app:assembleDebug`：通过。
- 相同样本的修复后截图：待复核。

## 2026-07-21：短按钮标签低对比度修复

提交：`9fb8b4a fix(rendering): preserve contrast for short control labels`

### 对比素材

原图局部：

![编辑按钮原图](assets/2026-07-21-edit-label-source.png)

异常结果：

![Edit 标签低对比度结果](assets/2026-07-21-edit-label-low-contrast.png)

### 根因

`Edit` 已被识别和绘制，并非 OCR 返回空值。由于中文 `编辑` 的 OCR 框很紧，文字像素在框内占比较高，旧算法可能把文字灰阶误判为背景主色，最终估算出与深色按钮接近的前景颜色，视觉上表现为标签消失。

### 修改内容

- 文字前景继续从 OCR 框内部提取。
- 背景主色改为从 OCR 框外围约三分之一文字高度的环形区域采样。
- 外围区域不可用时才回退到框内背景统计。
- 前景与背景亮度差小于 90 时启用可读性保护：深色背景使用白色，浅色背景使用黑色。
- 本轮不修改 OCR 候选融合，避免影响上一轮 `Share` 误识别修复。

### 验证

- `git diff --check`：通过。
- `:app:compileDebugKotlin`：通过。
- `:app:assembleDebug`：通过。
- 修复后 `Edit` 标签截图：待复核。

## 2026-07-21：清晰短中文未替换修复

提交：`44a3432 fix(translation): classify short Han labels deterministically`

### 对比素材

原图局部：

![分享按钮原图](assets/2026-07-21-share-untranslated-source.png)

异常结果：

![分享保持中文而其他按钮已翻译](assets/2026-07-21-share-untranslated-result.png)

### 根因

`分享` 已被 OCR 正确识别，颜色和绘制也正常；未替换发生在翻译阶段。Language Identification 对两个汉字的样本可能返回 `und` 或误判为其他东亚语言，导致中文 UI 术语没有执行，后续模型失败后触发了“保留原图”保护。相邻的 `编辑`、`删除` 恰好被正确判定，所以表现不一致。

### 修改内容

- 目标为英文且文本包含汉字时，优先执行已知中文 UI 术语匹配。
- 汉字脚本 OCR 直接归类为中文，不再把极短文本交给统计语言识别。
- 拉丁文、韩文等不含汉字的内容继续使用动态 Language Identification。
- 将中文 UI 规则提取为单一函数，避免语言识别前后存在两套不同逻辑。

### 验证

- `git diff --check`：通过。
- `:app:compileDebugKotlin`：通过。
- `:app:assembleDebug`：通过。
- 修复后 `Share` 截图：待复核。

## 2026-07-21：`<分淳` 异常与 OCR 共识融合

提交：`6e43543 feat(ocr): fuse recognition passes by consensus`

### 问题

深色按钮上的 `分享` 被识别为 `<分淳`。其中 `<` 来源于相邻图标或笔画边缘，`淳` 是 `享` 的形近字误识。原有一个字符容错无法同时承担“删除边界符号”和“替换形近字”两个编辑操作。

### 修改内容

- 短中文匹配前移除首尾非字母、数字、汉字的噪声字符。
- `<分淳` 先归一化为 `分淳`，再以一个字符编辑距离匹配 `分享`。
- 原图、灰度高对比和反色 OCR 从顺序覆盖改为候选聚类投票。
- 候选按区域重叠率聚类，并综合原图可靠度、字符质量和跨分支文本相似度评分。
- 只有增强分支发现的浅色区域孤立候选会被过滤。
- 深色区域孤立候选可以保留，以维持白字深底的召回率。
- 多个识别分支结果一致时获得额外评分，降低单次偶发误识覆盖正确结果的概率。

### 验证

- `git diff --check`：通过。
- `:app:compileDebugKotlin`：通过。
- `:app:assembleDebug`：通过。
- `<分淳` 样本重新识别结果：待真机复核。

## 2026-07-21：移除写死 UI 翻译

提交：`7071ae9 refactor(translation): remove hardcoded UI translations`

### 修改原因

`TranslateManager` 中维护具体中文 UI 词语及固定英文结果，会让样本效果依赖业务词表，无法泛化到未知图片，也会掩盖真实 OCR 错误。需求调整为所有识别文本均使用自动语言识别和 ML Kit 翻译。

### 修改内容

- 删除 `uiTranslations` 及全部固定中文到英文映射。
- 删除 UI 关键词包含判断和基于词表的编辑距离纠错。
- 所有非目标语言文本统一经过源语言识别、模型准备和 ML Kit Translation。
- 保留不涉及语义的通用 OCR 清理：含汉字且不超过 10 个字符的短文本，会去除首尾 `<`、`>`、`=`、`|`、`·`、`•` 噪声。
- 英文目标结果的汉字残留、异常首字符和有效字符比例检查保持不变。

### 取舍

- 新图片不再依赖预置词表，行为更通用。
- 孤立的 UI 短词缺少上下文，ML Kit 离线模型可能给出不符合界面语境的译文。
- OCR 形近字错误不再由翻译词表修正，必须依靠三路 OCR 共识融合提高源文本准确率。

### 验证

- 已确认翻译模块不存在 `uiTranslations`、`Share`、`Edit`、`Delete` 等固定结果。
- `git diff --check`：通过。
- `:app:compileDebugKotlin`：通过。
- `:app:assembleDebug`：通过。

## 2026-07-21：缩放文档小字识别与长句翻译失败

提交：`226032f fix(pipeline): upscale small text before OCR`

### 对比素材

手机画面中的缩放文档：

![手机画廊内缩小显示的文档](assets/2026-07-21-small-document-phone-view.png)

原始文档中箭头文字行：

![原始文档中的清晰文字行](assets/2026-07-21-small-document-source-line.png)

### 根因

原始文档中文字清晰，但在手机截图中整份文档被缩放到约 440 px 宽，实际 OCR 输入里的单字高度约为 5–8 px。当前流程只限制大图最长边，从不放大小图，未满足 ML Kit 建议的约 16×16 px 字符像素条件。技术文本还混有 `XMIND_WORKER_ID`、`Claude/Codex` 等标识符，模型可能保留少量源字符，旧的“英文结果不能含任何汉字”规则会使整行翻译失败。

### 修改内容

- 将 OCR 输入与 OpenCV/输出处理图分离。
- 最长边不足 1600 px 的图片，仅在 OCR 阶段临时放大，最大三倍且最长边不超过 3072 px。
- OCR 结束后按 X/Y 比例将识别框映射回处理图坐标，并立即释放临时 Bitmap。
- 长度不少于 16 的译文若首次质量检查失败，按中英文句号、分号、问号和感叹号切分后逐段重试。
- 英文结果允许最多 10% 汉字，兼容模型保留的少量专有内容；超过阈值仍判定失败。
- 只有翻译成功的区域进入 OpenCV 遮罩。失败区域保留原始像素，不执行擦除。

### 取舍

- 小图 OCR 的临时放大和三路识别会增加识别时间与峰值内存。
- 放大只服务于识别，最终图片尺寸不随 OCR 输入放大。
- 源图本身没有足够笔画像素时，插值放大不能恢复真实细节，只能帮助模型利用现有轮廓。

### 验证

- `git diff --check`：通过。
- `:app:compileDebugKotlin`：通过。
- `:app:assembleDebug`：通过。
- 箭头长句识别、翻译和擦除效果：待同图复核。

## 2026-07-21：原图分辨率替换与单点还原

提交：`fbc59e0 feat(editor): toggle translated regions on the original image`

### 目标

- 不再把最长边 2048 px 的 OCR 工作图作为最终输出。
- 在原图分辨率上执行 OpenCV 擦除和译文回填。
- 支持点击单个替换点查看擦除替换前的原始内容，并可再次切回译文。

### 修改内容

- OCR 临时图只用于识别；识别框按 X/Y 比例映射回原图坐标。
- OpenCV Inpaint、Canvas 绘制和保存均使用原始图片尺寸。
- 绘制时记录每段译文实际占用范围，并与原文字擦除范围合并。
- 结果图点击坐标通过 `ImageView.imageMatrix` 逆变换映射到原图坐标。
- 点击命中的最小替换区域时，从原图复制像素实现局部还原。
- 再次点击同一区域时，从译文区域快照恢复翻译效果。
- 保存操作直接保存当前可见状态，允许混合保留原文和译文。
- 为控制内存，不保留第三张完整译图，只为每个替换区域保存小尺寸译文 patch。
- 加载新图片或 Activity 销毁时主动回收区域 patch。

### 交互状态

- 首次点击译文区域：显示该区域原文。
- 再次点击同一区域：恢复该区域译文。
- 点击没有替换记录的位置：不修改图片。
- 外层滚动视图仍可截获拖动手势，单击用于区域切换。

### 取舍

- 原图分辨率 OpenCV 处理会比 2048 px 工作图使用更多时间和峰值内存。
- 区域还原采用矩形像素 patch，重叠译文区域可能存在相互覆盖，当前通过优先命中面积最小区域降低影响。
- 极高分辨率照片后续可增加“原图质量/内存优化”模式选择。

### 验证

- `git diff --check`：通过。
- `:app:compileDebugKotlin`：通过。
- `:app:assembleDebug`：通过。
- 原图分辨率、单点点击和保存当前状态：待真机交互复核。

## 2026-07-21：保守擦除与替换区域编号

提交：`5226c42 feat(editor): constrain inpainting and mark editable regions`

### 问题

- 精确遮罩的深色/浅色候选都不可信时，旧逻辑仍默认选择深色遮罩。
- 全局 5×5 膨胀会把文字附近的图标、边框或纹理纳入擦除范围。
- 虽然 OpenCV 理论上保持 mask 外像素，但旧代码直接把整张 Inpaint 输出转成 Bitmap，缺少逐像素不变的显式保证。
- 可点击还原区域没有视觉提示，用户无法快速判断哪些位置已被擦除重写。

### 擦除调整

- 精确遮罩候选的允许前景比例从 1%–55% 收紧为 1.5%–42%。
- 深色和浅色候选都不在可信范围时跳过该区域，不执行擦除。
- OCR 区域外围扩展从 3 px 收紧为 2 px。
- 形态学膨胀核由 5×5 收紧为 3×3。
- Telea 半径由 3.0 收紧为 2.0。
- 默认启用“精确遮罩”；矩形遮罩保留为手动备选。

### 原图合成保证

- 原始 RGBA Mat 克隆为最终合成底图。
- OpenCV 在 RGB 工作图上生成修复结果。
- 修复结果转换回 RGBA 后，只通过单通道文字 mask 复制到合成底图。
- mask 外像素直接来自原图，不使用 Inpaint 输出替代整张图片。

### 编号与还原提示

- 新增独立 `ReplacementOverlayView`，在替换区域左上角显示编号。
- 红色编号表示当前显示译文，绿色编号表示该区域已还原原文。
- 点击编号对应区域可在原文和译文之间切换。
- 新增“标号”复选框，可随时隐藏提示层。
- 编号层不绘制到 Bitmap，因此不会出现在保存图片中。

### 构建

- OpenCV 和 ML Kit 原生库使 Debug APK 约 246 MB，2 GB Gradle 堆在 Zipflinger 打包时发生 OOM。
- `org.gradle.jvmargs` 调整为 4 GB 后完整打包通过。

### 验证

- `git diff --check`：通过。
- `:app:compileDebugKotlin`：通过。
- `:app:compileDebugJavaWithJavac`：通过。
- `:app:assembleDebug`：通过。
- 编号位置、滚动手势和单点还原：待真机交互复核。

## 2026-07-21：图标误标与文字边界分离

提交：

- `a59f895 fix(markers): label only actual text replacements`
- `ded5d39 fix(ocr): isolate text elements from adjacent icons`

### 对比素材

原图：

![带前置图标的设置菜单](assets/2026-07-21-icon-prefix-source.png)

异常编号结果：

![图标误标和编号遮挡](assets/2026-07-21-marker-false-positives.png)

### 问题分类

1. 标号 8 对应图标或开关，被 OCR 当成单个拉丁字符/数字。翻译接口因源语言已经是英文而返回原文本，但旧 Activity 仍把调用成功视为翻译成功，进而擦除、重写并编号。
2. 标号 9–17 多数对应真实文字，但旧编号画在 OCR 框左上角，覆盖了图标或译文开头，形成类似缺字的视觉结果。
3. ML Kit 的 `Line.boundingBox` 有时同时包住前置图标、正文和尾部箭头。即使移动编号，使用整行框仍可能把图标纳入 OpenCV 遮罩。

### 实际替换判定

- 单个非汉字的拉丁字母、数字或符号 OCR 候选直接过滤。
- 译文去除首尾空白后必须与 OCR 原文不同，才标记为实际替换。
- 只有实际替换区域会进入 OpenCV mask、Canvas 绘制、还原 patch 和编号列表。
- 目标语言原本就是英文且模型返回原文本时，不擦除、不重写、不编号。
- 翻译失败和无需翻译分别统计，状态栏不再把无需翻译内容计为失败。

### 编号位置

- 编号半径由 10 dp 减少为 8 dp。
- 编号统一放在图片左侧提示栏，Y 坐标与替换行中心对齐。
- 编号不再覆盖前置图标和译文首字。

### element 级文字边界

- 从 ML Kit `Line` 下钻到 `Element`，不再直接使用整行文本和整行边界。
- 按横向间距将 elements 划分为连续组，超过约 0.4 个行高的空隙视为分组边界。
- 选择有效文字字符总量最大的组作为正文。
- 组首或组尾为孤立单字符、且与正文间距明显时，将其作为图标噪声剔除。
- 中英混合长句根据元素间距恢复必要空格。
- 最终 OCR 文本边界为正文 elements 的矩形并集，直接控制擦除、重写、点击还原和编号。

### 验证

- `git diff --check`：通过。
- `:app:compileDebugKotlin`：通过。
- `:app:assembleDebug`：通过。
- 标号 8 消失、9–17 图标保留和编号关联：待同图真机复核。

## 2026-07-21：标识符字号与品牌 Logo 保护

提交：`2797829 fix(translation): preserve identifiers and target-script brands`

### 对比素材

![用户名标识符与品牌 Logo 被编号](assets/2026-07-21-identifier-logo-markers.png)

### 标号 4 分析

标号 4 对应 `49280224点`，内容以数字为主体，更接近用户名、账号或业务标识符，而不是自然语言。自动翻译会把末尾汉字扩展为英文单词，使字符串明显变长，原胶囊区域无法容纳时字号被缩小。对标识符执行翻译和重排在语义与视觉上都不合理。

### 标号 15 分析

Logo 中的 `marshub × Titan Network` 确实是可识别文字，因此 ML Kit OCR 检出它本身合理。但目标语言已经是英文，品牌专名不应因为短文本语言识别误判而再次翻译，也不应执行 OpenCV 擦除。该场景应作为“检测到文字但无需替换”处理。

### 修改内容

- 目标语言为英文时，完全由拉丁字母、数字和符号组成且包含拉丁字母的文本直接保留。
- 数字不少于 4 个、最多包含一个汉字、且数字占有效字符至少 65% 时，判定为标识符并保留。
- 保留发生在语言识别和模型翻译之前。
- 返回原文后，Activity 的“译文必须与原文不同”规则会阻止擦除、重写、patch 和编号。

### 取舍

- 规则保护英文品牌、代码、用户名、账号和产品名。
- 当前目标固定为英文，因此拉丁脚本的法语、西班牙语等内容也会被视为目标脚本并保留。若后续需要多语言到英文，应在 UI 中明确选择源语言，或增加高置信度语言检测模式。

### 验证

- `git diff --check`：通过。
- `:app:compileDebugKotlin`：通过。
- `:app:assembleDebug`：通过。
- 标号 4、15 消失且原图像素保持：待同图真机复核。

## 2026-07-21：译文样式估算与兼容字体

提交：`9c0b02c feat(rendering): infer source text style for translations`

### 能力边界

ML Kit Text Recognition 不返回字体名称、字号、字重、字距或斜体等排版元数据。位图也无法可靠反推出原字体文件，因此实现采用像素级样式估算，并使用 Android 字体 fallback，而不是声称恢复精确字体。

### 可估算样式

- 文字前景颜色：根据 OCR 框外围背景主色与框内高差异像素计算。
- 深浅背景：用于对比度保护和控件标签判断。
- 字号层级：根据每个 OCR 区域的字高恢复，标题和正文分别计算。
- 常规/粗体：根据文字框内与背景存在明显差异的笔画像素覆盖率估算。
- 对齐：深色控件标签保持居中，普通页面文字保持左对齐。
- 代码倾向：不含汉字并带下划线、URL 或代码字符组合时使用等宽 fallback。

### 字体选择

- 普通文本使用 Android `sans-serif`。
- 代码型文本使用 Android `monospace`。
- 根据笔画覆盖率选择 `NORMAL` 或 `BOLD`。
- 原字体缺失或译文包含原字体不支持的字符时，由 Android 系统字体链自动补齐。
- 含汉字的普通菜单不会被代码启发式误判为等宽字体。

### 字号调整

- 常规文字首选字号约为 OCR 字高的 1.12 倍。
- 粗体文字首选字号约为 OCR 字高的 1.05 倍。
- 最小字号提高到 OCR 字高的 0.68 倍。
- 最终仍受可用绘制区域约束，避免译文覆盖相邻内容。

### 无法可靠恢复的样式

- 精确字体家族和字体版本。
- 字体 hinting、字偶距和 OpenType 特性。
- 斜体角度、下划线、删除线和文字阴影。
- 渐变、纹理、描边和逐字多色效果。

### 验证

- `git diff --check`：通过。
- `:app:compileDebugKotlin`：通过。
- `:app:assembleDebug`：通过。
- 标题、正文、粗体和颜色的视觉匹配程度：待样本截图复核。

## 2026-07-21：下载记录图标与译文重叠

提交：`7514515 fix(ocr): trim icon ink from short label bounds`

### 对比素材

正确原图：

![下载记录和房间设置原图](assets/2026-07-21-download-icon-source.png)

异常结果：

![下载记录译文覆盖前置图标](assets/2026-07-21-download-icon-overlap.png)

### 对比分析

- `房间设置` 的 ML Kit element 边界从正文起点开始，因此 `Room setting` 正确绘制在图标右侧。
- `下载记录` 的前置图标与正文被合并到同一个 element，整段边界从图标左侧开始。
- 图标还可能被识别为类似“亮”的伪字符，使模型输出类似 `Light download record`。
- 使用该边界擦除和绘制时，译文会覆盖原图图标。

### 修改内容

- 对 2–10 个字符且包含汉字的短标签执行前景墨迹列投影。
- 背景颜色从 OCR 框外围采样，框内与背景色差足够大的列视为前景墨迹。
- 按超过约 0.18 个行高的空列间距拆分图标组和文字组。
- 选择宽度最大的墨迹组作为正文边界；要求它至少比第二大组宽 20%。
- 新边界只在文字组左右保留约十二分之一行高的安全 padding。
- 根据分离出的首尾组宽度估算伪字符数量。
- 只有 OCR 字符数超过正文墨迹组的估算字符容量时，才移除首尾伪字符；否则仅收紧边界，不修改文本。
- 长句和超过 10 个字符的内容不使用该规则，避免把正常词间距误判为图文分隔。

### 验证

- `git diff --check`：通过。
- `:app:compileDebugKotlin`：通过。
- `:app:assembleDebug`：通过。
- 标号 7 图标保留和 `Download record` 起始位置：待同图复核。

## 2026-07-21：短中文部分翻译与上下文重试

提交：`b3482e3 fix(translation): retry partial short UI translations`

### 对比素材

点击还原后的原文：

![自然色与启用自动旋转原文](assets/2026-07-21-partial-short-label-original-toggle.png)

部分翻译结果：

![自Although与启用Automatic rotation](assets/2026-07-21-partial-short-label-output.png)

### 根因

标号 4 和 9 已被 OCR 检测并进入翻译流程，并非完全漏识别。ML Kit 对缺少上下文的短 UI 词只翻译了部分内容，产生 `自Although` 和 `启用Automatic rotation`。旧规则允许英文长结果保留最多 10% 汉字，边界值使这类短文本可能被接受并写回。

### 修改内容

- 中文到英文且输入不超过 12 个字符时，译文必须包含零个汉字。
- 首次结果残留汉字时，以通用 `界面设置选项：<原文>` 上下文重新调用 ML Kit。
- 重试结果从最后一个中英文冒号后提取，去掉通用上下文部分。
- 上下文仅提供文本类型，不包含任何中文词语到英文结果的写死映射。
- 长技术句继续允许最多 10% 汉字，兼容专名或模型保留内容。
- 若上下文重试仍包含汉字，则判定失败并保留原图，不执行 OpenCV 擦除。

### 诊断增强

- 每个可还原区域保存 OCR 原文和最终译文。
- 点击编号切换原文/译文时，状态栏显示 `OCR 原文 → 最终译文`。
- 后续可直接判断异常来自 OCR 源文本还是翻译模型输出。

### 验证

- `git diff --check`：通过。
- `:app:compileDebugKotlin`：通过。
- `:app:assembleDebug`：通过。
- `自然色`、`启用自动旋转` 的上下文重试结果：待同图复核。

## 2026-07-21：擦除、重写、标号与排版链路一致性

提交：`2369129 fix(pipeline): align safe erasure with text rendering`

### 综合历史现象

- 图标或非文字被纳入 OCR 边界时，精确蒙版可能擦除过多。
- 精确蒙版因前景比例异常而拒绝某段文字后，旧流程仍按“翻译成功”绘制译文，可能出现原文与译文叠加。
- 标号过去只依赖翻译状态，不能严格表示该区域已经完成“擦除并重写”。
- `下载记录` 一类短设置项的译文可向整张图片右侧扩展，存在遮挡行尾箭头、开关等未识别控件的风险。
- 多轮 OCR 已用于候选融合，但融合结果没有保存识别次数与一致性信息，点击还原时难以判断异常来自单轮识别还是多轮共识。

### 根因

- `ImageInpainter` 只返回处理后的位图，没有把实际通过蒙版安全检查的区域返回给界面层。
- 界面层以 `translation != source` 作为擦除、绘制、标号和还原区域的共同条件；这个条件无法反映 OpenCV 是否真的擦除了该区域。
- 普通文字布局默认最多向右扩展到画布边缘、向下扩展三个行高，未对短 UI 标签预留尾部控件区。
- OCR 融合在选择最佳候选后丢弃了通过次数和文本相似度等诊断信息。

### 修改内容

- 新增 `InpaintResult`，同时返回原图合成结果和实际安全擦除的原始文字区域。
- 精确蒙版仅在前景占比通过 `1.5%..42%` 检查时登记为已擦除；没有任何安全区域时直接复制原图，不调用 OpenCV Inpaint。
- 译文写入、编号标记、点击还原快照全部只基于 `erasedRegions`，确保编号严格表示已经完成擦除和重写的区域。
- 被精确蒙版拒绝的区域保留原图，完成状态中单独报告“因擦除风险保留原文”的数量。
- OCR 融合结果新增 `consensusScore` 与 `passCount`；分数综合原图/增强/反色通过次数、候选文本相似度和文本质量。
- OCR 共识仅用于点击区域后的诊断显示，不设置强制淘汰阈值，避免清晰短中文只在单轮识别成功时再次漏识别。
- 2–16 字符的短 UI 标签在右侧至少预留三个行高或画布宽度 10% 的空间，并将额外垂直空间从三个行高收紧为一个行高。
- 已识别到的同行和下一行文字仍作为排版边界，和右侧固定预留共同避免译文覆盖图标、箭头、开关及下一设置项。

### 预期效果

- 精确擦除失败时不再出现原文未擦除却写入译文的双层文字。
- 标号、点击还原和实际替换区域保持一一对应。
- `下载记录`、`房间设置`、`启用自动旋转` 等短标签的英文译文不会一直延伸到行尾控件。
- 点击标号可看到 OCR 原文、最终译文、共识百分比和识别通过次数，便于后续区分 OCR、翻译、擦除和排版问题。

### 验证

- `git diff --check`：通过。
- `:app:compileDebugKotlin`：通过；仅保留既有 `MediaStore.getBitmap` 弃用警告。
- `:app:assembleDebug`：通过。
- 历史截图中的图标保留、短中文翻译和右侧控件避让：待同图真机复核。

## 2026-07-21：中文、英文单向翻译与逐项双向互译

提交：`13eb191 feat(translation): support Chinese English direction modes`

### 需求解释

- “单项翻译”按与“双向互译”相对应的单向翻译模式实现。
- 提供 `中 → 英`、`英 → 中`、`中 ⇄ 英` 三种模式。
- OCR 仍以每个识别片段为独立翻译单元；双向模式可在同一张中英混合图片中逐项选择相反目标语言。

### 修改内容

- `TranslateManager` 新增 `TranslationMode`，不再把目标语言固定为英文。
- 单向模式只翻译符合当前方向的片段，另一种语言、纯数字和不可翻译符号保持原图。
- 双向模式中，含汉字片段使用中文到英文模型；拉丁字母片段使用英文到中文模型。
- 启动时同时准备中英和英中模型，实际 Translator 仍按语言对缓存并复用。
- 英文短词识别为 `und` 或其他拉丁语言时，在中英产品范围内回退为英文，避免 `Color`、`Edit` 等短 UI 词漏翻。
- 英文到中文的 32 字符以内短标签新增 `UI setting option: <原文>` 上下文重试。
- 短英文译文必须包含中文；拉丁字符超过汉字两倍时判定为不完整翻译并保留原图。
- 数字编号、URL、邮箱、下划线代码和含 `×/©/®/™` 的品牌组合继续跳过翻译，避免破坏既有图标和品牌保护。
- 底部新增单选分段控件，默认选择 `中 ⇄ 英`；翻译执行期间锁定三个模式按钮，保证整批 OCR 片段使用同一模式。

### 与现有链路的关系

- 各模式最终仍通过“译文确实变化 → 安全擦除成功 → 写入 → 标号 → 点击还原”的统一链路。
- 目标方向不匹配、翻译结果未变化或验证失败的片段不会擦除原图，也不会生成标号。
- 双向翻译不会绕过之前的 OCR 共识、图标分离、样式估算和短标签控件避让规则。

### 验证

- `git diff --check`：通过。
- `:app:compileDebugKotlin`：通过；仅保留既有 `MediaStore.getBitmap` 弃用警告。
- `:app:assembleDebug`：通过。
- Debug APK 覆盖安装到 USB Pixel 3：成功。
- Pixel 3（1080×2160）UI Automator 检查：三个模式可选，默认双向，模式栏和底部操作栏无重叠。
- 启动日志无崩溃和 OpenCV 异常；存在 Firebase Installations 网络超时，已缓存的本地翻译模型仍可使用。

## 2026-07-21：标号点击查看 OCR 原文与译文浮层

提交：`558b3f6 feat(markers): show OCR translation details in popup`

### 输入与现象

- 旧逻辑点击替换区域后，把 `OCR 原文 → 译文 → 共识信息` 写入页面顶部 `tvStatus`。
- 长文本会让状态栏换行增高，挤压或遮挡图片选择按钮和图片查看区域。
- 标号只负责绘制，不处理自身点击；点击编号圆点时不一定落入实际文字矩形，因此可能没有响应。

### 修改内容

- `ReplacementOverlayView` 为每个编号计算与绘制一致的屏幕圆心。
- 编号显示半径保持 8dp，点击热区扩大到 18dp，提升小标号命中率。
- 覆盖层只在编号热区内消费触摸；其他位置返回未处理，继续透传给结果图片的原文/译文切换手势。
- 点击编号后显示独立 `PopupWindow`，内容包含编号、OCR 原文、翻译文字和 OCR 共识信息。
- 浮层宽度上限 300dp、高度上限 250dp，距离屏幕边缘至少 16dp，并根据编号纵向位置自动限制在可见屏幕内。
- 原文和译文区域可滚动、可选择；长文本不会改变主页面任何控件尺寸。
- 浮层支持关闭按钮和点击外部关闭；关闭标号显示、加载新图、重新处理或销毁页面时自动关闭。
- 点击译文区域仍只切换原图/译图，不再修改顶部状态文本。

### 验证

- `git diff --check`：通过。
- `:app:compileDebugKotlin`：通过；仅保留既有 `MediaStore.getBitmap` 弃用警告。
- `:app:assembleDebug`：通过。
- Debug APK 覆盖安装到 USB Pixel 3：成功，新增资源可正常加载，启动无崩溃。
- 真机重新安装会清空内存中的替换区域，本轮未能自动复用旧标号完成点击截图；需使用同图重新翻译后复核浮层内容与位置。

## 2026-07-21：macOS 底图与译文上下翻转修复

提交：未提交（当前工作区）

### 输入与现象

- macOS 系统擦除和 OpenCV 擦除生成的结果图都曾出现上下翻转，说明异常位于两个引擎之后的公共渲染层。
- 将公共底图恢复正向后，擦除位置正确，但重新写入的翻译文字仍然上下翻转。
- 左右位置始终正确，问题只涉及图像 Y 方向、区域坐标原点和文字字形方向。

### 根因与排查过程

- Vision 原始识别框使用左下角归一化坐标；`OCRService` 已将其转换为左上角像素坐标，后续模型和 SwiftUI 覆盖层都按这套逻辑坐标工作。
- `ImageRenderer` 为了直接使用左上角矩形，把共享 AppKit 位图上下文设为 `flipped: true`。上下文翻转会同时影响底图、遮罩和文字，并不只是把矩形原点从左下角改为左上角。
- 初次排查根据“水平校正后得到 180 度旋转”推断 OpenCV 输出上下翻转，并在 OpenCV 分支增加了专属 Y 翻转。这个推断忽略了 macOS 系统引擎也经过同一个翻转上下文，因此把公共渲染问题错误归因到了引擎输出。
- 随后先在正常上下文绘制底图、再用翻转上下文绘制覆盖层，解决了结果图片方向，但 AppKit 字形继续受到覆盖层变换影响，所以译文仍然倒置。
- 最终根因不是 OpenCV、图片方向元数据或 Vision 框本身，而是把坐标系适配实现成了整个绘图状态的变换。

### 最终修改

- 移除 OpenCV 输出上的专属 Core Image 翻转，两个引擎返回的底图均保持原始方向。
- 移除公共渲染层的 `flipped: true` 上下文。
- 底图、系统遮罩和译文全部在正常 AppKit 位图上下文中绘制。
- 逻辑区域继续保存左上角坐标；进入 AppKit 绘制时单独转换矩形：`drawY = imageHeight - sourceBounds.maxY`。
- OpenCV mask、背景颜色采样和 SwiftUI 标号继续使用原始左上角区域，避免在不同阶段重复转换。
- 完整架构流程、坐标表和防回归约束见 [macOS 图像处理流程与坐标约定](../macOS/README.md#图像处理流程与坐标约定)。

### 验证

- 2×2 角点颜色测试确认旧共享翻转会将上下两行互换；移除后底图行顺序保持不变。
- 仓库真实 PNG 在无替换区域时经过新渲染层，输入与输出像素差 `same_sad=0`；与上下翻转结果的差值为 `vertical_flip_sad=34798534`。
- AppKit 文字方向测试使用顶部红色标记和 `TRANSLATION UP` 文本，确认文字位于顶部区域且字形正向。
- `xcodebuild` Debug 配置构建成功。
- 当前机器未安装 Homebrew OpenCV，OpenCV 配置无法完成最终链接；OpenCV 输入/输出 PNG 边界测试保持原始行方向，实际 Inpaint 运行仍需安装依赖后复核。

## 2026-07-21：中英文双 OCR 融合与翻译前人工复核

提交：`09e9243 feat(ocr): add dual-script manual review workflow`

### 输入与现象

- 英文新闻样本尺寸为 401×390；原中文识别器虽然能识别部分英文，但标题第一行只得到 `U.S.strikes Iran after`，遗漏 `attacks on`。
- 第二行能够得到 `vessels in Strait of Hormuz`，说明问题不是图片整体分辨率不足，而是中文模型对长英文行的覆盖不稳定。
- OCR 错误一旦直接进入翻译、擦除与回写阶段，就会放大为不完整译文和错误擦除，单纯继续调高自动过滤阈值会增加清晰文字漏识别。

### OCR 修改

- 在中文 ML Kit Text Recognition 之外增加 Latin Text Recognition；原图、灰度增强图和深色区域反色图均由两个模型识别。
- 候选融合加入 ML Kit 行置信度、识别脚本适配度、有效字符完整度和候选框宽度覆盖率，避免较短但字符质量较高的残缺候选胜出。
- 识别次数按“模型 + 图像通道”统计，并记录最终候选来自中文、拉丁或双模型融合。
- 对同一基线、字号接近且水平间距不超过 0.75 个行高的相邻片段做保守拼接，避免将正文的不同排版行合并。
- 增加仅 Debug 可用的 `OcrDiagnosticActivity`，可输出区域、融合共识、通过次数、模型置信度和识别脚本，便于后续固定样本回归。

### 人工复核流程

- 主操作从一次性“翻译”拆分为“识别文字”和“确认翻译”两个阶段。
- 识别完成后，结果区显示未经修改的原图副本，并为每个 OCR 候选显示编号；此阶段不调用 OpenCV，也不会擦除或重绘图片。
- 点击编号弹出局部浮层，可查看和修改 OCR 文字，查看模型置信度、融合共识和识别脚本，也可取消“参与翻译”。
- 参与翻译的候选显示红色编号；排除后显示绿色编号并保留原图。修改仅在点击“应用”后更新当前候选。
- 点击“确认翻译”后，只将人工保留的候选送入既有翻译、安全擦除、样式估算、译文写入和可还原区域链路。
- 翻译完成后的编号仍使用原有详情浮层，显示最终采用的 OCR 原文和译文；点击译文区域仍可在原图像素和译图像素之间切换。

### 识别结果

同一英文新闻图在 Pixel 3 上以 3 倍 OCR 图（1203×1170）执行诊断：

```text
中文单模型：U.S.strikes Iran after
双模型融合：U.S. strikes Iran after attacks on

中文单模型：vessels in Strait of Hormuz
双模型融合：vessels in Strait of Hormuz
```

第一行缺失的 `attacks on` 已补全，第二行在融合完整度评分加入后没有被较短的 `Strait of Hormuz` 候选覆盖。

### 验证

- `git diff --check`：通过。
- 使用 Android Studio JBR（Java 17+）执行 `:app:assembleDebug`：通过；仅保留既有 `MediaStore.getBitmap` 弃用警告。
- Debug APK 覆盖安装到 USB Pixel 3（设备 `8APX0WVAU`）：成功。
- `ImageTranslateActivity` 启动成功；UI Automator 确认“识别文字”入口可见、翻译模式可选、未生成结果前保存按钮禁用。
- 启动日志无 AndroidRuntime 崩溃和 OpenCV 异常。
- 按要求暂不继续 2 倍/3 倍缩放与更多样本的自动对照；后续通过编号复核浮层人工确认误识别、漏识别和排除区域。

## 2026-07-22：完整中文短标签识别与顶部按钮遮挡修复

提交：`0b267bc fix(ocr): preserve complete Chinese labels`

### 输入与现象

- Pixel 3 原始设置截图 `Screenshot_20260717-102902_设置.png` 中，`自然色`、`启用自动旋转`、`已开启 / 时钟` 均清晰可见。
- 最终替换详情分别只保存了 `然色`、`自动旋转`、`已开启/`，导致原图残留 `自`、`启用`、`时钟`。
- 三个残片的 OCR 共识仍为 89%–90%，说明多通道共识只能表示候选一致，不能表示整行覆盖完整。
- 顶部滚动区域只约束到状态文字底部，而“选择图片”按钮更高；滚动区域后绘制并覆盖了按钮下半部分。

### 根因

- `refineLine()` 将 ML Kit 行按视觉间距分组后只保留有效字符最多的一组，错误丢弃同一设置项的中文前缀或后缀。
- `refineShortLabelByInk()` 会选择最宽墨迹组并按估算字数裁剪；普通中文字距也可能超过旧阈值，因此把真实汉字当成相邻图标。
- 旧算法需要保护行首图标，不能简单取消全部裁边，否则垃圾桶等图标会再次被识别为汉字并进入擦除区域。

### 修改内容

- 行分组保留所有至少包含两个有效字符的文字组；没有合格组时才回退到字符最多的一组。
- 同一行存在 `/`、`／`、`|`、`｜`、`·` 时，在多个文字组之间恢复显式分隔符。
- 墨迹裁剪增加有效字符覆盖率保护：默认至少保留原文字的 80%。
- 仅在“恰好剔除一个字符且与正文存在至少 0.28 个行高的明显间距”时，允许低于 80% 的图标裁剪。
- 组内单字符图标继续使用原有严格间距剔除，避免完整文字修复重新包含工具栏图标。
- 顶部状态文字改为填充按钮左侧剩余宽度，并增加 8dp 间距。
- 新增底部 Barrier 同时引用状态文字和“选择图片”按钮；滚动区域从两者共同底边下方 8dp 开始。

### 同图 OCR 回归

| 标号 | 修复前 | 修复后 | 边界变化 |
| --- | --- | --- | --- |
| 4 | `然色` | `自然色` | 左边界 `104 → 71` |
| 9 | `自动旋转` | `启用自动旋转` | 左边界 `182 → 68` |
| 13 | `已开启/` | `已开启/时钟` | 右边界 `182 → 294` |

第二张包含通知、缩略图和底部操作图标的原图回归结果：

- `自然色`、`启用自动旋转` 保持完整。
- `分享`、`编辑` 仍未包含左侧图标。
- 垃圾桶误识别字符 `日` 被排除，删除文字区域恢复为 `x=725..808`，没有扩展到图标所在的 `x=650`。

### 构建与界面验证

- `git diff --check`：通过。
- 使用 Android Studio JBR 执行 `:app:assembleDebug --rerun-tasks`：通过；仅保留既有 `MediaStore.getBitmap` 弃用警告。
- Debug APK 覆盖安装到 USB Pixel 3：成功。
- UI Automator 实测“选择图片”按钮范围为 `y=121..253`，滚动区域从 `y=275` 开始，保留 22px 间隔，不再重叠。

## 2026-07-22：粗体文字擦除放行与密集标号避让

提交：`56ea13d fix(pipeline): handle bold text and marker collisions`

### 输入与现象

- 测试图尺寸为 385x590；`标准翻译`、`高精翻译` 为清晰的黑色粗体中文，但结果中没有稳定完成擦除和回写。
- 左侧编号 10-17 的纵向间距小于编号圆的直径，多个编号显示和点击区域互相覆盖。
- 原测试截图已经包含一组旧编号；真机验证时需滚动到应用结果层，或关闭“标号”开关后对照，避免把图中旧编号误判为当前覆盖层。

### 根因判断

- OCR 诊断确认两处标题均已正确识别：`标准翻译` 的模型置信度为 0.73，`高精翻译` 为 0.71，因此不是 ML Kit 漏识别。
- 精确遮罩统计中，`标准翻译` 的深色前景占比为 0.412，旧阈值下可以擦除；`高精翻译` 为 0.453，超过固定上限 0.42，导致擦除被拒绝，后续也不会回写或生成替换标号。
- 标号原来全部固定在同一横坐标，仅使用 OCR 区域中心纵坐标；密集小行在图片缩放后会发生视觉碰撞。

### 修改内容

- 精确遮罩继续对普通和近方形区域使用 0.42 的前景占比上限；仅对宽高比不小于 1.8 的横向文字区域放宽到 0.48。
- 新增精确遮罩诊断数据，Debug OCR 页面可选执行翻译和擦除链路，并输出深色/浅色前景占比及最终放行结果。
- 标号在最终 View 坐标中按纵向位置排序；发生碰撞时依次进入右侧横向轨道，轨道间距大于圆直径。
- 标号点击从“第一个命中”改为“最近中心点命中”，解决多轨道点击区域相邻时选错编号的问题。

### 验证

- Pixel 3（设备 `8APX0WVAU`）管线诊断：`标准翻译 -> Standard translation`，深色占比 0.412，擦除通过。
- Pixel 3 管线诊断：`高精翻译 -> High-translation`，深色占比 0.453，修复后擦除通过。
- 完整 UI 链路共识别 28 段、完成替换 24 段；状态栏未报告翻译失败或擦除风险保留原文。
- 滚动到结果层检查时，密集编号已分配到多个横向轨道，圆形不再互相覆盖。
- `git diff --check` 通过；使用 Android Studio JBR 执行 `:app:assembleDebug --rerun-tasks` 构建成功，仅保留既有 `MediaStore.getBitmap` 弃用警告。

### 残留观察

- `高精翻译` 当前动态译文为 `High-translation`，识别和擦除已经正常，但语义更自然的 `High-precision translation` 属于翻译模型质量问题，可在后续上下文翻译调校中单独处理。
- 测试截图本身含有旧编号；后续视觉回归优先保留一份无标号原图，减少覆盖层对照干扰。

## 2026-07-22：英文新闻长图回归验证

基线提交：`56ea13d fix(pipeline): handle bold text and marker collisions`

### 输入与范围

- 输入为 1080x2400 的英文新闻截图，SHA-256 为 `dcc8177323dc5396d787ce9096ecfd9a3594554fd2628cb0e5e6a141699891f4`。
- 图片长边已经超过 1600px，OCR 直接使用原始像素，不执行额外放大。
- 在 Pixel 3（设备 `8APX0WVAU`）验证 OCR 诊断、人工复核、双向翻译、精确遮罩、矩形遮罩和标号显示。

### OCR 与标号结果

- 共得到 11 个 OCR 候选；照片主体、黑色空白区和底部操作图标没有产生候选。
- 新闻标题第一行完整识别为 `U.S. strikes Iran after attacks on`，第二行为 `vessels in Strait of Hormuz`。
- 两行标题的融合共识均为 1.00、识别次数均为 4、模型置信度均为 0.87；此前缺失的 `attacks on` 没有再次丢失。
- 同一行的标号 9、10 自动分配到两个横向轨道，没有发生圆形覆盖。
- 顶部状态栏字符、社交图标中的 `in` 和底部极小图片说明也被识别；这些是真实字符，但是否翻译需要在人工复核阶段排除。

### 完整双向翻译结果

- 默认保留全部 11 个候选后，完成 6 段替换，3 段翻译失败；运行期间没有 AndroidRuntime 崩溃或 OpenCV 异常。
- 新闻照片、头像及上下黑色区域保持原图，没有发生非文字区域擦除。
- 标题译文出现语义问题：`U.S. strikes Iran after attacks on` 被译为 `攻击后，美国罢工伊朗`，`vessels in Strait of Hormuz` 被译为 `海峡海峡的船只`。
- 图片说明存在源 OCR 小字误差，例如 `Oil -> OI`、`Qaboos -> Oaboos`，译文形成中英混排。

### 遮罩对照

- 开启精确遮罩时，两行标题右侧仍能看到浅色的原英文笔画，说明抗锯齿边缘没有完全进入前景掩膜。
- 关闭精确遮罩并使用相同 OCR 边界重新处理后，标题原文被完整清除，证明标题 OCR 边界完整，残影来自精确遮罩覆盖不足。
- 该样本不能判定为全量通过：OCR 完整性、标号避让和非文字保护通过；英文翻译质量、大号抗锯齿文字精确擦除和小号说明文字精度仍需后续处理。

## 2026-07-22：中文设置面板回归验证

基线提交：`56ea13d fix(pipeline): handle bold text and marker collisions`

### 输入与范围

- 输入为 1125x2436 的中文设置面板截图，SHA-256 为 `c46459b6d9edb317cca92514af2e1c770c9f01a0950a7fcbce4a6c2fdb4cb593`。
- 图片直接使用原始分辨率，在 Pixel 3（设备 `8APX0WVAU`）执行 OCR 诊断、人工复核、双向翻译、精确遮罩、标号详情和区域还原。

### OCR 结果

- 共得到 19 个 OCR 候选。
- 正确识别的主体菜单包括 `下载记录`、`房间设置`、`翻译设置`、`播放设置`、`显示设置`、`词汇与术语设置`。
- 与图标或帮助按钮相邻的标签仍有错误：`暗黑模式 -> 發暗黑模莫式`、`仅转写 -> 转写0`、`显示发言人 -> 示发言人の`。
- `一键清空数据` 被识别为 `一键请空数据`，属于单字近形误识别。
- 顶部信号图标被识别为 `0ll令0`；底部品牌被识别为重叠候选 `台marshub x、 Titan Network` 和 `Titan Network`。

### 标号与翻译结果

- 相邻标号 7、8 以及底部标号 14、15 均进入横向轨道，没有发生圆形覆盖；标号详情浮层可正常显示 OCR 原文、译文和共识信息。
- 默认保留全部候选后完成 15 段替换，无翻译失败、OpenCV 异常或应用崩溃。
- 正确菜单项得到可读译文，且其左侧图标、右侧箭头和开关保持原图。
- 错误 OCR 被直接放大为错误译文：`暗黑模式 -> Dark black mold`、`仅转写 -> Turnover 0`、`显示发言人 -> Spokesman`、`一键清空数据 -> One-click request`。
- 品牌 Logo 被擦除并叠加 `Marshub X, Titan Network` 与 `泰坦网络`，不符合品牌区域应保留原图的预期。

### 还原验证与结论

- 点击标号 14 可查看品牌候选详情；点击对应译文区域后，原始 `marshub x Titan Network` 像素恢复，标号 14 变绿。
- 同一品牌区域仍存在标号 15 的重叠候选且保持红色，说明重叠替换区域的显示状态和还原状态不能同步。
- 该样本不能判定为通过：标号避让、普通菜单图标保护和单区域还原通过；图标邻接文字完整性、品牌/状态栏过滤、重叠候选去重与近形汉字精度仍需处理。

### 标号 4 图标遮挡专项复检

- 全图 OCR 直接处理 1125x2436 长图时不执行放大，将该行识别为 `發暗黑模莫式`，边界为 `x=54..298`，把左侧图标误识别为首字并纳入擦除区域。
- 将包含图标和正文的 420x160 行区域裁剪并放大 3 倍后，OCR 准确输出 `暗黑模式`；候选框映射回原图约为 `x=131..302`，正确排除了约位于 `x=54..120` 的图标。
- 仅保留正文的 300x120 裁剪样本同样准确输出 `暗黑模式`，说明字符模型本身能够识别该标签。
- 裁剪样本执行翻译与精确遮罩时擦除通过，深色前景占比为 0.244、浅色占比为 0.319；因此标号 4 遮挡图标的根因是长图小字未局部放大，而不是 OpenCV 遮罩主动扩张。
- 当前动态译文为 `Diablo mode`，仍不是期望的 `Dark mode`，属于短标签翻译语义问题。
- 后续修复应为长图增加局部切片/二阶段放大 OCR，并把局部坐标映射回原图；不应对所有候选统一右移边界，否则会裁掉没有图标的正常文字。

## 2026-07-22：长图短标签局部 OCR 复检

提交：`2997f8f fix(ocr): refine small labels in long images`

### 修改内容

- 长边不小于 1600px 时，对高度 16..72px、长度 2..12 且包含中文的候选执行局部二阶段 OCR。
- 局部区域使用左侧 0.5 个行高、右侧 3 个行高、上下各 1 个行高的非对称上下文，最多放大 3 倍且长边不超过 1536px。
- 局部复检只运行中文模型的原图和灰度对比度通道，避免对完整英文长句增加额外开销。
- 局部结果通过坐标比例映射回原图，并结合垂直重叠、边界重叠、字符保留率、文本相似度和模型置信度决定是否替换全图候选。
- 如果局部中文只是原中文的严格短子串，则拒绝替换，避免再次把 `启用自动旋转` 裁成 `用自动旋转`。
- 任一局部识别失败时保留全图候选，不影响原有多模型、多通道结果。

### 标号 4 结果

| 项目 | 修复前 | 修复后 |
| --- | --- | --- |
| OCR 文字 | `發暗黑模莫式` | `暗黑模式` |
| 原图边界 | `x=54..298` | `x=130..303` |
| 图标范围 | 被候选覆盖 | 约 `x=54..120`，已排除 |
| 模型置信度 | 0.43 | 0.51 |
| 精确擦除 | 包含图标 | 仅正文，图标完整保留 |

- 完整 UI 中标号 4 的译文从正文起点写入，左侧圆形图标没有被擦除或遮挡。
- 整图仍完成 15 段替换，无翻译失败、OpenCV 异常或 AndroidRuntime 崩溃。
- 动态译文仍为 `Diablo mode`；本提交只修复 OCR 文字和边界，未把翻译词条写死。

### 回归结果

- 同一设置图保持 19 个候选；`用户名、`、`下载记录`、`房间设置`、`翻译设置`、`播放设置`、`显示设置`、`词汇与术语设置` 未被裁短。
- 英文新闻图仍完整识别 `U.S. strikes Iran after attacks on` 和 `vessels in Strait of Hormuz`，两行置信度均为 0.87。
- 历史系统设置图保持 `自然色`、`启用自动旋转`、`已开启/时钟` 完整；其中 `启用自动旋转` 边界恢复为 `x=68..298`。
- `转写0`、`示发言人。`、`一键请空数据`、品牌 Logo 和顶部状态栏过滤仍是已知问题，没有在本次专项修复中扩大处理范围。
- `git diff --check` 通过；Android Studio JBR 执行 `:app:assembleDebug` 成功，Debug APK 已覆盖安装并在 Pixel 3 验证。

## 后续记录模板

每次调校追加以下内容，不覆盖已有记录：

```markdown
## YYYY-MM-DD：调校主题

提交：`<commit> <message>`

### 输入与现象
- 使用的原图/设备/模式
- 观察到的具体差异

### 根因判断
- 代码或算法层面的原因

### 修改内容
- 参数、算法和涉及文件

### 验证
- 构建结果
- 原图与结果图
- 已改善项和残留项
```
