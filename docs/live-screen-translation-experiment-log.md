...
> 建立日期：2026-07-26
>
> 当前代码基线：`7b028b7 perf: enable differential OCR for large scrolls`
>
...
视频参考方案使用浏览器 DOM 翻译，可以直接替换节点并触发页面重排。当前项目是跨应用 OCR 覆盖方案，不能控制底层页面布局，因此目标是接近它的交互节奏，而不是假设可以完全复刻 DOM 重排：滚动中显示原页面，稳定后快速、完整地提交新译文。
...
| 录屏会话 | 验证结束后关闭 |
| 无障碍增强 | 验证结束后关闭 |
| 当前推荐下一项 | 自动化同视口 A/B + 结构化性能/覆盖率日志 |
